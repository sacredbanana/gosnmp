...
func (t *TrapListener) SendUDP(packet *SnmpPacket, addr *net.UDPAddr) error {
	...
	if count != len(ob) {
		t.Params[0].Logger.Printf("Failed to send all bytes of SnmpPacket!\n")
	}
	return nil
}
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				t.Params[0].Logger.Printf("TrapListener: error in read %s\n", err)
				continue
			}
...
func (t *TrapListener) handleTCPRequest(conn net.Conn) {
	...
	msg := buf[:reqLen]
	for _, params := range t.Params {
		traps, err := params.UnmarshalTrap(msg, false)
		if err != nil {
...
func (t *TrapListener) debugTrapHandler(s *SnmpPacket, u *net.UDPAddr) {
	t.Params[0].Logger.Printf("got trapdata from %+v: %+v\n", u, s)
}
